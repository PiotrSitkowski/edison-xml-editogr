...
<table class="table table-sm table-inv-lines">
  ...
  {foreach from=$XMLData['Invoice-Lines']['Line'] item=item key=key}
    <tr>
      ...
      <td scope="col"><input type="text" name="Invoice-Lines[Line][{$key}][Line-Item][BuyerItemCode]" value="{$item['Line-Item'].BuyerItemCode}" style="width:100% !important;"></td>
      ...
      <td scope="col"><input type="text" name="Invoice-Lines[Line][{$key}][Line-Item][ItemDescription]" value="{$item['Line-Item'].ItemDescription}" style="width:100% !important;"></td>
    </tr>
  {/foreach}
